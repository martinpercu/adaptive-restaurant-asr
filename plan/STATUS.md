...
**Next: Phase 5 — Axis 3 (Keydetector).** Menu lexicon (fuzzy + phonetic matching) + growing confusion-pair rule engine (seeded from `configs/confusion_seed.yaml`), golden test framework (positive + negative per rule), replace/log_only modes. Gate: KER improvement, false-correction rate ≤ 0.5%. This is fully CPU-feasible (deterministic rules, no training). Read `plan/phases/phase-5-axis3-keydetector.md`. MUSAN/CB music still deferred; phase-4 GPU candidate still pending.
